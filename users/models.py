...
class Profile(models.Model):
    ...
    private_list = models.ForeignKey(Video, on_delete=models.SET_NULL, null=True, blank=True)
    public_list = models.ForeignKey(Video, on_delete=models.SET_NULL, null=True, blank=True)
    email = models.EmailField(max_length=200,null=True, blank=True)
    short_info = models.CharField(max_length=200, null=True, blank=True)
    followers = models.ManyToManyField(User,related_name="followers", blank=True)
    history = models.ManyToManyField(Video, blank=True)
    favorite_video = models.ManyToManyField(Video, blank=True)
